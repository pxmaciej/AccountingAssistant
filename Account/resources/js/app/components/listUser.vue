...
<v-table :data="users"
     ...
     <tbody slot="body" slot-scope="{displayData}">
         <tr v-for="users in displayData"  :key="users.id">
              ...
              <td>{{users.role}}</td>
              <td>{{users.company}}</td>
              <td><button class="btn btn-danger mr-1" type="button" @click="deleteUser(users.id)" >Usuń</button><a href="/selecteduser"><button class="btn btn-success mr-1" type="button" @click="selectedUser(users.id)">Wybierz</button></a><a href="/reset"><button class="btn btn-primary" type="button" @click="selectedUser(users.id)">Profil</button></a></td>
         </tr>
     </tbody>
</v-table>
<smart-pagination
